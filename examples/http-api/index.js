...
async function main() {
    ...
    const server = new core_1.MCPServer({
        ...
        version: '1.0.0',
        plugins: [
            new core_1.LoggingPlugin(),
            new core_1.MetricsPlugin(),
        ],
    });
    // Set HTTP transport with CORS enabled
    server.setTransport(new transport_http_1.HttpTransport({
        host: '127.0.0.1',
        port: 3000,
        enableCors: true,
    ...
    server.registerTool({
        ...
        inputSchema: core_1.z.object({
            text: core_1.z.string().describe('Text to analyze'),
        }),
        handler: async (input) => {
    ...
    server.registerTool({
        ...
        inputSchema: core_1.z.object({
            json: core_1.z.string().describe('JSON string to validate'),
            prettify: core_1.z.boolean().optional().describe('Whether to prettify output'),
        }),
        handler: async (input) => {
            try {
    ...
    server.registerTool({
        ...
        inputSchema: core_1.z.object({
            numbers: core_1.z.array(core_1.z.number()).describe('Array of numbers'),
            operation: core_1.z.enum(['sum', 'average', 'min', 'max', 'sort']).describe('Operation to perform'),
        }),
        handler: async (input) => {
