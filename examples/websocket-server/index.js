...
const transport_websocket_1 = require("@mcp-accelerator/transport-websocket");
async function main() {
    // Create server
    ...
    server.setTransport(new transport_websocket_1.WebSocketTransport({
        host: '127.0.0.1',
        port: 3001,
    }));
    ...
    server.registerTool({
        ...
        inputSchema: core_1.z.object({
            a: core_1.z.number().describe('First number'),
            b: core_1.z.number().describe('Second number'),
        }),
        handler: async (input) => {
            return {
                operation: 'addition',
    ...
    server.registerTool({
        ...
        inputSchema: core_1.z.object({
            base: core_1.z.number().describe('Base number'),
            exponent: core_1.z.number().describe('Exponent'),
        }),
        handler: async (input) => {
            return {
                operation: 'power',
    ...
    server.registerTool({
        ...
        inputSchema: core_1.z.object({
            message: core_1.z.string().describe('Message to broadcast'),
        }),
        handler: async (input) => {
