...
server.registerMiddleware({
    ...
    async handler(message, context, next) {
        const apiKey = context.metadata?.['x-api-key'];
        const authHeader = context.metadata?.['authorization'];
        try {
            // Try API Key first
            if (apiKey) {
                const apiKeyMiddleware = (0, middleware_auth_1.createAPIKeyAuthMiddleware)({
                    keys: API_KEYS,
                    getUserInfo: (key) => ({
                        type: 'service',
                        apiKey: key,
            ...
            // Try JWT second
                ...
                const jwtMiddleware = (0, middleware_auth_1.createJWTAuthMiddleware)({
                    secret: JWT_SECRET,
                    algorithms: ['HS256'],
...
server.registerTool({
    ...
    inputSchema: core_1.z.object({
        fields: core_1.z.array(core_1.z.string()).optional()
    }),
    handler: async (input, context) => {
        const user = context.metadata?.user;
...
server.registerTool({
    ...
    inputSchema: core_1.z.object({
        action: core_1.z.string(),
        target: core_1.z.string().optional()
    }),
    handler: async (input, context) => {
        const user = context.metadata?.user;
