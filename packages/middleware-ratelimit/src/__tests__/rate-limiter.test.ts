import { createRateLimitMiddleware, createRedisStore, RateLimitStore } from '../rate-limiter';
...
describe('Rate Limiter Middleware', () => {
  ...
  describe('Middleware Handler - Basic Functionality', () => {
    ...
    it('should reject requests exceeding limit', async () => {
      ...
      await expect(middleware.handler(message, context, mockNext)).rejects.toThrow(
        'Rate limit exceeded. Maximum 2 requests per 60 seconds.'
      );
  ...
  describe('Key Generator', () => {
    it('should use custom key generator', async () => {
      const keyGenerator = jest.fn((context) => context.metadata.userId as string);
      const middleware = createRateLimitMiddleware({
        max: 2,
        windowMs: 60000,
        keyGenerator,
    ...
    it('should rate limit by user ID from metadata', async () => {
      const middleware = createRateLimitMiddleware({
        max: 1,
        windowMs: 60000,
        keyGenerator: (context) => context.metadata.user?.id as string || context.clientId,
      });
      ...
      const contextUser1: MiddlewareContext = {
        clientId: 'client-1',
        logger: mockLogger,
        metadata: {
          user: { id: 'user-1' },
      ...
      const contextUser2: MiddlewareContext = {
        clientId: 'client-2',
        logger: mockLogger,
        metadata: {
          user: { id: 'user-2' },
  ...
  describe('Skip Function', () => {
    ...
    it('should skip based on user role', async () => {
      const skip = (context: MiddlewareContext) => {
      ...
      const adminContext: MiddlewareContext = {
      ...
      const userContext: MiddlewareContext = {
  ...
  describe('Redis Store', () => {
    it('should create Redis store', () => {
      ...
      const store = createRedisStore(mockRedis, { windowMs: 60000 });
